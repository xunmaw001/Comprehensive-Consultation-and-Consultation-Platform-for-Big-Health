                        ...
                        <form id="addOrUpdateForm">
                            <div class="form-row">
                                   ...
                                   <div class="form-group col-md-6">
                                       <label>资讯医生</label>
                                       <select id="ysTypesSelect" name="ysTypes" class="form-control">
                                       </select>
                                   </div>
                                   <div class="form-group col-md-6">
                                       <label>资讯用户</label>
                                   ...
                                   <div class="form-group  col-md-6">
                                       <label>资讯内容</label>
                                       <input type="text" class="form-control" name="informationContent" id="informationContent-input" >
                                   </div>
...
<script>
    ...
    // 文件上传
    function upload() {
        $('#informationContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('informationContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("jiaoliu/" + urlParam, "POST", data, (res) => {
                if(res.code == 0
        )
            {
    ...
    //查询当前页面下所有列表
        function ysTypesSelect() {
            //填充下拉框选项
            http("yisheng/page?page=1&limit=100&sort=&order=&dicCode=ys_types", "GET", {}, (res) => {
                if(res.code == 0){
    ...
    //初始化下拉框
    function initializationYstypesSelect(){
        var ysTypesSelect = document.getElementById('ysTypesSelect');
            for (var i = 0; i < ysTypesOptions.length; i++) {
                ysTypesSelect.add(new Option(ysTypesOptions[i].yishenggonghao,ysTypesOptions[i].id));
            }
    }
    function initializationYhtypesSelect(){
        var yhTypesSelect = document.getElementById('yhTypesSelect');
            for (var i = 0; i < yhTypesOptions.length; i++) {
                yhTypesSelect.add(new Option(yhTypesOptions[i].yonghuming,yhTypesOptions[i].id));
            }
    }
    // 下拉框选项回显
    function setSelectOption() {
        for (var i = 0; i < ysTypesOptions.length; i++) {
            if(ysTypesOptions[i].id == ruleForm.ysTypes){//下拉框value对比,如果一致就赋值汉字
                document.getElementById("ysTypesSelect").options[i].selected = true;
            }
        }
        for (var i = 0; i < yhTypesOptions.length; i++) {
    ...
    function dataBind() {
        ...
        $("#informationContent-input").val(ruleForm.informationContent);
    ...
    $(document).ready(function () {
        var id = window.sessionStorage.getItem("updateId");
        if(id != null){
            $("#ysTypesSelect").attr('style', 'pointer-events: none;');
            $("#yhTypesSelect").attr('style', 'pointer-events: none;');
            $("#informationContent-input").attr("readonly",true);
        }
